...
class VgrecUploader < CarrierWave::Uploader::Base
#include SessionsHelper
 ...
 #def store_dir
  ...
  def zencode(args)
        Rails.logger.warn "zencoding_VG"
        #b/c not a M/V/C, use this instead of logger.warn
        # @time = Digest::SHA1.hexdigest([Time.now, rand].join)[4..8].upcase.to_s  #can't use in filename b/c would need to set same @time in filename method below
            #transcode the file as a genuine mp3 via Zencoder
        zencoder_response = Zencoder::Job.create({
                      :api_key => ENV['ZEN_API_KEY'],    
                      :input => "s3://#{ENV['VG_BUCKET_NAME']}/#{model.user_id.to_s}_#{model.id.to_s}.mov",
                      :outputs => [
                        {
                          :url => "s3://#{ENV['VG_BUCKET_NAME']}/#{model.user_id.to_s}_#{model.id.to_s}.mp3",
                          :skip_video => true,
                          #:clip_length => "10",
                          :audio_codec => "mp3",
                          :public => 1,
   ...
   @model.zen_job_id = zencoder_response.body['id']  #saves the Zencoder job ID to the rec_file_size column of the practice object
